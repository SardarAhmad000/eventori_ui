...
class VendorBookingController extends GetxController {
  ...
  Rxn<EventVendor> eventVendorModel=Rxn<EventVendor>();
...
// Method to set evidence image
  void setEvidenceImage(File? image) {
    ...
    if (image != null) {
      ...
      showEvidenceError.value = false; // Hide error when image is selected
      print('Image path set: ${imagePath.value}');
    }
  }
...
// Method to remove evidence image
  ...
  // Flag to control when to show evidence error
  final RxBool showEvidenceError = false.obs;
  ...
  String? _validateImage(File? image) {
    if (image == null) {
      return 'Please upload an Evidence  image';
    }
    return null;
  }

  Future reportVendor(String vendorName, String reason, String description, String evidence,String vendorId) async {
    ...
    response = await DataApiService.instance
        .multiPartImage('vendor/report',[imagePath.value],'evidence', body)
        .catchError((error) {
  ...
  void submitReport() {
    ...
    print('╠═══════════════════════════════════════════════════════════╣');
    print('║ Evidence:                                                 ║');
    print('║ Image Path: ${imagePath.value.padRight(40)}║');
    print('║ Has Image: ${(imagePath.value.isNotEmpty ? 'Yes' : 'No').padRight(40)}║');
    print('╠═══════════════════════════════════════════════════════════╣');
    print('║ Metadata:                                                 ║');
