...
class DataApiService {
    ...
    try {
      ...
      if (imageFile.isNotEmpty) {
        for (int i = 0; i < imageFile.length; i++) {
          if (imageFile[i] != null) {
            print(imageFile);
            request.files.add(await http.MultipartFile.fromPath(fieldName, imageFile[i]!));
          }
        }
      }
      // request.files
      ...
      http.StreamedResponse response =
          await request.send().timeout(const Duration(seconds: TIME_OUT_DURATION));
  ...
  async {
    ...
    try {
      ...
      if (imageFile != null) {
        request.files.add(await http.MultipartFile.fromPath(fieldName, imageFile));
      }
      ...
      case 500:
        {
          ...
          //     description: 'You have been logged out of this device, Please Sign In to continue.',
          //     onTap: () async {
          //       Get.back();
          //       // await authController.logoutUser("token");
          //       // AuthPreference.instance.setUserLoggedIn(false,"barber");
